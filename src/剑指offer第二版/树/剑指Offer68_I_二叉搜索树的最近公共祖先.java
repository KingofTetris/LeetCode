...
public class 剑指Offer68_I_二叉搜索树的最近公共祖先 {
    ...
    public TreeNode lowestCommonAncestor(TreeNode root, TreeNode p, TreeNode q) {
        if(root.val < p.val && root.val < q.val) //比两个值都小，那么去右子树找
            return lowestCommonAncestor(root.right, p, q);
        if(root.val > p.val && root.val > q.val) //比两个值都大，那么去右子树找
            return lowestCommonAncestor(root.left, p, q);
        return root; //如果一大一小，那OK，这个root就是他们的根节点
    ...
    public TreeNode lowestCommonAncestor2(TreeNode root, TreeNode p, TreeNode q) {
        while(root != null) {
            ...
            else break; //找到一大一小了。那么这个节点就是他们的最近公共祖先
        }
        return root;
