    ...
    public boolean sequenceReconstruction(int[] nums, int[][] sequences) {
        ...
        for (int i = 0; i < sequences.length; i++) {
            ...
            for (int j = 0; j < sequence.length - 1; j++) {
                ...
                //如果还没有这个点的记录，就添加一个空的列表
                adjacency.putIfAbsent(from,new LinkedList<>());
                //把to放进去，同时要记得to的入度+1
        ...
        //按顺序入队，如果Sequences里面包含单个顺序[1]，那它虽然没进入上面的循环，但也由于数组的初始化而为0
        ...
        while (!queue.isEmpty()){//如果queue不为空就一直删除入度为0的点
            //每次入队后，入度为0的点不止一个那相对顺序就不可能唯一，也就无法获得唯一的最短超序列
            //直接返回false;
            if (queue.size() > 1) return false;

            int from = queue.poll();//队头元素出队 队列长度-1
            //取出队头元素指向的点列表，删除from-to的每条边
            LinkedList<Integer> toList = adjacency.get(from);
            if (toList == null) continue; //如果这个点根本没有指向下一个点比如 [1] 那就跳过。
